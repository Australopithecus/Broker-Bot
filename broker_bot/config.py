...
from .bots import AI_LAB_BOT_NAME, LLM_BOT_NAME, ML_BOT_NAME, STAT_ARB_BOT_NAME, normalize_bot_name
...
class Config:
    ...
    ai_lab_alpaca_api_key: str
    ai_lab_alpaca_secret_key: str
    ai_lab_alpaca_paper_url: str
    ai_lab_alpaca_data_feed: str
    universe_path: str
    db_path: str
    model_dir: str
    reports_dir: str
    ...
    ai_lab_lookback_days: int
    ai_lab_symbol_limit: int
    ai_lab_max_long_positions: int
    ai_lab_max_short_positions: int
    ai_lab_min_abs_score: float
    ai_lab_exploration_rate: float
...
def load_config() -> Config:
    ...
    ai_lab_api_key = os.getenv("ALPACA_AI_LAB_API_KEY", "").strip()
    ai_lab_secret_key = os.getenv("ALPACA_AI_LAB_SECRET_KEY", "").strip()
    ai_lab_paper_url = os.getenv("ALPACA_AI_LAB_PAPER_URL", paper_url).strip() or paper_url
    ai_lab_data_feed = os.getenv("ALPACA_AI_LAB_DATA_FEED", data_feed).strip() or data_feed
    ...
    ai_lab_policy_path = os.getenv("AI_LAB_POLICY_PATH", "data/ai_lab_policy.json").strip()
    ...
    return Config(
        ...
        ai_lab_alpaca_api_key=ai_lab_api_key,
        ai_lab_alpaca_secret_key=ai_lab_secret_key,
        ai_lab_alpaca_paper_url=ai_lab_paper_url,
        ai_lab_alpaca_data_feed=ai_lab_data_feed,
        ...
        ai_lab_policy_path=ai_lab_policy_path,
        ...
        ai_lab_lookback_days=int(os.getenv("AI_LAB_LOOKBACK_DAYS", "260")),
        ai_lab_symbol_limit=int(os.getenv("AI_LAB_SYMBOL_LIMIT", "180")),
        ai_lab_max_long_positions=int(os.getenv("AI_LAB_MAX_LONG_POSITIONS", "10")),
        ai_lab_max_short_positions=int(os.getenv("AI_LAB_MAX_SHORT_POSITIONS", "6")),
        ai_lab_min_abs_score=_champion_override(
            AI_LAB_BOT_NAME,
            "ai_lab_min_abs_score",
            float(os.getenv("AI_LAB_MIN_ABS_SCORE", "0.006")),
            0.0,
            0.05,
        ),
        ai_lab_exploration_rate=float(os.getenv("AI_LAB_EXPLORATION_RATE", "0.10")),
...
def get_bot_account_config(config: Config, bot_name: str | None = None) -> BrokerAccountConfig:
    ...
    if normalized == AI_LAB_BOT_NAME:
        if not config.ai_lab_alpaca_api_key or not config.ai_lab_alpaca_secret_key:
            raise RuntimeError(
                "Missing ALPACA_AI_LAB_API_KEY or ALPACA_AI_LAB_SECRET_KEY in environment for the AI Lab bot."
            )
        return BrokerAccountConfig(
            bot_name=AI_LAB_BOT_NAME,
            api_key=config.ai_lab_alpaca_api_key,
            secret_key=config.ai_lab_alpaca_secret_key,
            paper_url=config.ai_lab_alpaca_paper_url,
            data_feed=config.ai_lab_alpaca_data_feed,
        )
    return BrokerAccountConfig(
