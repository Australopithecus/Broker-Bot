...
from .ai_lab_bot import rebalance_ai_lab_bot
from .bots import AI_LAB_BOT_NAME, LLM_BOT_NAME, ML_BOT_NAME, STAT_ARB_BOT_NAME
...
def cmd_caretaker_ai_lab(args: argparse.Namespace) -> None:
    _cmd_caretaker_for_bot(AI_LAB_BOT_NAME)
...
def cmd_rebalance_ai_lab(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
    review_summary = "Learning skipped."
    try:
        review_report = review_and_learn(config, bot_name=AI_LAB_BOT_NAME)
        review_summary = review_report.summary
    except Exception as exc:
        review_summary = f"Learning skipped: {exc}"
    symbols = _load_symbols(config)
    result = rebalance_ai_lab_bot(config, symbols)
    if result.orders:
        log_trades(config.db_path, result.orders, bot_name=AI_LAB_BOT_NAME)
    log_decision_run(
        config.db_path,
        result.ts,
        float(result.decision_context.get("effective_leverage", 0.0)),
    ...
    ts_pos, positions = snapshot_positions(config, bot_name=AI_LAB_BOT_NAME)
    log_positions(config.db_path, ts_pos, positions, bot_name=AI_LAB_BOT_NAME)
    spy_value = fetch_latest_close(config, "SPY", bot_name=AI_LAB_BOT_NAME)
    ts_eq, equity, cash, port = snapshot_equity(config, bot_name=AI_LAB_BOT_NAME)
    log_equity(config.db_path, ts_eq, equity, cash, port, spy_value=spy_value, bot_name=AI_LAB_BOT_NAME)
    print(
        f"AI LAB rebalanced at {result.ts} with {len(result.orders)} orders. "
        f"Learning summary: {review_summary}"
    )
...
def cmd_snapshot_ai_lab(args: argparse.Namespace) -> None:
    _cmd_snapshot_for_bot(AI_LAB_BOT_NAME)
...
def cmd_review_decisions_ai_lab(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
    report = review_and_learn(config, bot_name=AI_LAB_BOT_NAME)
    print(f"{report.headline}: {report.summary}")
    if report.report_path:
        print(f"Saved report to {report.report_path}")
...
def cmd_strategy_report_ai_lab(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
    report = generate_strategy_report(config, bot_name=AI_LAB_BOT_NAME)
    print(f"{report.headline}: {report.summary}")
    print(f"Saved report to {report.report_path}")
...
def cmd_attribution_report_ai_lab(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
    report = generate_attribution_report(config, bot_name=AI_LAB_BOT_NAME)
    print(f"{report.headline}: {report.summary}")
    print(f"Saved report to {report.report_path}")
...
def cmd_champion_report_ai_lab(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
    report = generate_champion_challenger_report(config, bot_name=AI_LAB_BOT_NAME)
    print(f"{report.headline}: {report.summary}")
    print(f"Saved report to {report.report_path}")
...
def cmd_summary_report(args: argparse.Namespace) -> None:
    config = load_config()
    init_db(config.db_path)
